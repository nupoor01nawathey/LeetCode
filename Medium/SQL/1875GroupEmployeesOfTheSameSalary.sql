...
create table employee(employee_id int, name text, salary int);
...
WITH cte AS (
  SELECT
    *,
    COUNT(employee_id) OVER(PARTITION BY salary) employees_with_same_salary
  FROM employee
)
SELECT 
  employee_id, 
...
FROM cte
WHERE employees_with_same_salary >= 2
