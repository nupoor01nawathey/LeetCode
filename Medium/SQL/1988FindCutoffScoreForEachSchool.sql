...
-- Each row in this table indicates that there are student_count students that got at least 
-- score points in the exam.
-- The data in this table will be logically correct, meaning a row recording a higher 
-- score will have the same or smaller student_count compared to a row recording a 
-- lower score. More formally, for every two rows i and j in the table, if scorei > scorej 
-- then student_counti <= student_countj.
-- Every year, each school announces a minimum score requirement that a student needs to 
-- apply to it. The school chooses the minimum score requirement based on the exam 
...
-- If there are multiple score values satisfying the above conditions, choose the smallest 
-- one. If the input data is not enough to determine the score, report -1.
...
create table school(school_id int, capacity int);
create table exam(score int, student_count int);

insert into school values(11,151);
insert into school values(5,48);
insert into school values(9,9);
insert into school values(10,99);
